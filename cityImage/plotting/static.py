    ...
    def __init__(self, figsize, black_background, fontsize, title=None):
        ...
        if title is not None:
            self.fig.suptitle(
                ...
                fontsize=self.font_size_primary,
...
def plot_gdf(
    ...
    if base_map_gdf is not None and not base_map_gdf.empty:
        _plot_base_map(
            gdf=base_map_gdf,
            ax=ax,
            color=base_map_color,
            geometry_size=base_map_geometry_size,
            alpha=base_map_alpha,
    ...
    if cbar:
        _generate_colorbar(
            ...
            cbar_max_symbol=cbar_max_symbol,
            cbar_min_max=cbar_min_max,
            cbar_shrink=cbar_shrink,
        )
    elif legend:
        _generate_legend_ax(ax, plot)

    return plot.fig


def plot_grid_gdfs_column(
    gdfs=None,
    column=None,
    ncols=1,
    nrows=1,
    ...
    if geometry_size_columns is None:
        geometry_size_columns = []
    if titles is None:
        titles = []
    if gdfs is None:
        gdfs = []
    ...
    for n, ax in enumerate(multiPlot.grid.flat):
        ...
        parameters = {
            ...
            "geometry_size": geometry_size,
            "geometry_size_columns": geometry_size_columns,
            "geometry_size_factor": geometry_size_factor,
...
def plot_grid_gdf_columns(
    ...
    if len(columns) == 0:
        raise ValueError(
            "Provide a list of columns to plot the geometries on. For a plain plot, use plot_gdf"
        )
    ...
    one_legend = scheme == "User_Defined"
...
def _plot_on_ax(
    ...
    ----------
    ax: matplotlib.axes object
        The axes object on which to create the _subplot.
    gdf: GeoDataFrame
        GeoDataFrame to be plotted.
    column: str
        Column on which the plot is based.
    ...
    if geometry_type == "Point":
        ...
        if geometry_size_factor is not None:
            geometry_size = [
                (abs(value) * geometry_size_factor)
                if (abs(value) * geometry_size_factor) > 1.1
                else 1.1
                for value in gdf[geometry_size_column]
            ]
...
def _subplot(
    ...
    """
    Create a _subplot with a map plot on the given axes.
    ...
    if unique_categories is None:
        unique_categories = []
    ax.set_aspect("equal")
    _set_axes_frame(axes_frame, ax, multiPlot.black_background, multiPlot.text_color)
    ...
    if titles:
        ax.set_title(
            ...
            loc="center",
            fontfamily="Times New Roman",
            fontsize=multiPlot.font_size_primary,
            color=multiPlot.text_color,
            pad=15,
        )
    ...
    if legend_fig and one_legend:
        _generate_legend_fig(ax, multiPlot, gdf, unique_categories, cmap)
    elif legend_ax:
        _generate_legend_ax(ax, multiPlot)

    multiPlot.fig.tight_layout()  # Adjust layout after adding elements


def _plot_base_map(gdf=None, ax=None, color=None, geometry_size=None, alpha=0.5, zorder=0):
    """
    ...
    """
    if (gdf.iloc[0].geometry.geom_type == "LineString") or (
        gdf.iloc[0].geometry.geom_type == "MultiLineString"
    ):
    ...
    if (gdf.iloc[0].geometry.geom_type == "Polygon") or (
        gdf.iloc[0].geometry.geom_type == "MultiPolygon"
    ):
        gdf.plot(ax=ax, color=color, alpha=alpha, zorder=zorder)


def _generate_legend_fig(ax, plot, gdf, unique_categories=None, cmap=None):
    """
...
def _generate_legend_ax(ax, plot):
    """
...
def _generate_colorbar(
    ...
    ax = plot.ax if isinstance(plot, Plot) else plot.grid
    ...
    if cbar_max_symbol:
        cb.ax.set_yticklabels(
            [round(t, 1) if t < norm.vmax else "> " + str(round(t, 1)) for t in cb.ax.get_yticks()]
        )

    plt.setp(
        plt.getp(cb.ax, "yticklabels"),
        color=plot.text_color,
        fontfamily="Times New Roman",
        fontsize=plot.font_size_secondary,
    )
