...
# def test_plot():
    ...
    # base_map_dict = {'gdf_base_map': edges_gdf_susa, 'base_map_alpha' : 0.4, 'base_map_geometry_size' : 1.1, 'base_map_zorder' : 0}
    ...
    # if 'secondary_road' in list(barriers_gdf['barrier_type'].unique()):
        # colors.append('gold')
    # cmap = LinearSegmentedColormap.from_list('new_map', colors, N=len(colors)) 
    # plot = ci.plot_gdf(barriers_gdf, column = 'barrier_type', geometry_size = 1.1, legend = True, axes_frame = False, black_background = False, cmap = cmap)                          
    ...
    # plot_flat = ci.plot_gdf(buildings_gdf)
...
def test_landmarks():
    ...
    sight_lines = ci.compute_3d_sight_lines(nodes_gdf_susa.iloc[:5], buildings_gdf, distance_along = 300,
    ...
    # scores    
    buildings_gdf = ci.structural_score(buildings_gdf, buildings_gdf, edges_gdf_susa, advance_vis_expansion_distance = 100, neighbours_radius = 100)   
    ...
    buildings_gdf = ci.compute_local_scores(buildings_gdf, l_cW, l_iW, rescaling_radius = 1500)
