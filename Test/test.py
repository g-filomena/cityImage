...
def test_load():
    ...
    network_fromOSM_place = ci.get_network_fromOSM(place, 'OSMplace', network_type = "all", epsg = epsg, distance = None)
    network_fromOSM_polygon = ci.get_network_fromOSM(OSMPolygon, 'OSMpolygon', network_type = "all", epsg = epsg, distance = None)
    network_fromOSM_address = ci.get_network_fromOSM(address, 'distance_from_address', network_type = "all", epsg = epsg, distance = 2000)
    dict_columns = {"roadType_field": "highway",  "direction_field": "oneway", "speed_field": None, "name_field": "name"}    
    network_fromSHP =  ci.get_network_fromSHP(path, epsg, dict_columns = dict_columns, other_columns = [])
...
def test_barriers(): 
    ...
    road_barriers = ci.road_barriers(place, download_method, distance = None, epsg = epsg, include_primary = True):
    water_barriers = ci.water_barriers(place, download_method, distance = None, epsg = epsg)
    railway_barriers = ci.railway_barriers(place, download_method, distance = None, epsg = epsg, keep_light_rail = True):
    park_barriers = ci.park_barriers(place, download_method, distance = None, epsg = epsg, min_area = 100000):
    barriers_gdf = pd.concat([road_barriers, water_barriers, railway_barriers, park_barriers])
    # assign barriers
...
def test_centrality():
    ...
    sc = ci.straightness_centrality(G, weight = 'lenght', normalized = True)
    ...
    rc = reach_centrality(G,  weight = 'lenght', radius = 400, attribute = 'services')
    
    Eb = nx.edge_betweenness_centrality(G, weight = 'length', normalized = False)
    edges_gdf = ci.append_edges_metrics(edges_gdf, G, dicts, ['Eb'])

def test_clean_network():
    nodes_gdf, edges_gdf = ci.get_network_fromOSM(place, 'OSMplace', network_type = "all", epsg = epsg, distance = None)    
    nodes_gdf, edges_gdf = ci.clean_network(nodes_gdf, edges_gdf, dead_ends = True, remove_disconnected_islands = True, same_uv_edges = True, self_loops = True, fix_topology = True)
...
def test_regions():
    ...
    reset_index_dual_gdfsIG(nodesDual_gdf, edgesDual_gdf):
