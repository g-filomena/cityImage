...
def get_fromSHP(path, epsg, crs, area = None, distance_from_boundary_area = 2000, roadType_field = None, direction_field = None, speed_field = None, name_field = None):
	...
	The "area" polygon is optional and when provided is used to select geometries within the area + a buffer of x meters, fixed by the researcher (distance_from_boundary_area)
    ...
    else: streets_gdf = streets_gdf[streets_gdf.geometry.within(area.buffer(distance_from_boundary_area)]
	...
	streets_gdf['geometry'] = streets_gdf.apply(lambda row: LineString([coor for coor in [row['geometry'].coords[i][0:2] for i in range(0, len(row['geometry'].coords))]])
	streets_gdf['from'] = streets_gdf.apply(lambda row: row.geometry.coord[0])
	streets_gdf['to'] = streets_gdf.apply(lambda row: row.geometry.coord[-1])
...
def dual_gdf(nodes_gdf, edges_gdf, crs):
	...
	centroids['intersecting'] = centroids.apply(lambda row: list(centroids_gdf.loc[(centroids_gdf['u'] == row['u'])|(centroids_gdf['u'] == row['v'] )|(centroids_gdf['v'] == row['v'] )|(centroids_gdf['v'] == row['u')].index))
	...
	edges_dual['deg'] = edges_dual.apply(lambda row: uf.ang_geoline(edges_gdf.loc[row['u']].geometry, edges_gdf.loc[row['v']].geometry, degree = True, deflection = True)
	edges_dual['rad'] = edges_dual.apply(lambda row: uf.ang_geoline(edges_gdf.loc[row['u']].geometry, edges_gdf.loc[row['v']].geometry, degree = False, deflection = True)
